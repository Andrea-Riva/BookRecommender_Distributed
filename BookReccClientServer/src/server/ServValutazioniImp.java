...
public class ServValutazioniImp  implements ServiziValutazioni {
    ...
    public  boolean aggiungiValutazione(int userId, int idLibro, int votoStile, int votoContenuto, int votoGradevolezza,
        ...
        if(userId<0){
            try {
                throw new UserIDError("errore nell'id utente");
            }catch (UserIDError e){
                System.out.println(e.getMessage());
            }
            return false;
        };
        String query = "INSERT INTO ValutazioniLibri (id_utente, id_libro, voto_stile, voto_contenuto, voto_gradevolezza, voto_originalità, voto_edizione," +
                "note_stile, note_contenuto, note_gradevolezza, note_originalità, note_edizione, voto_complessivo)" +
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
